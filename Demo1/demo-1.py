...
lcd.clear()
lcd.color = [0, 0, 0]  # Set initial color (red)

# The default window size is x = 640 and y = 480

currentAngle = None # stores the current angle found
lastAngle = None # stores the last detected angle to detect change
...
def find_marker_center(markerArr):
    markerArr = markerArr.flatten()
    markerCenter = 0
    return markerCenter

def find_marker_angle(markerCenter):
    return markerCenter
...
##    if frameCount % frames == 0:
    ...
    if ids is not None:  # if any markers are detected
        idCenter = find_marker_center(ids)
        currentAngle = find_marker_angle(idCenter)
